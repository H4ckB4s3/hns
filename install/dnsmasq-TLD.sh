...
auth-soa=$tld, root.$tld, $(date +%Y%m%d)01, 10800, 3600, 604800, 3600
...
\$ORIGIN $tld.
\$TTL 3600
@ IN SOA ns1.$tld. admin.$tld. $(date +%Y%m%d%H%M%S) 10800 3600 604800 3600
@ IN NS ns1.$tld.
ns1 IN A $ipv4
ns1 IN AAAA $ipv6
@ IN A $ipv4
...
auth-zone=$tld
auth-soa=$tld, root.$tld, $(date +%Y%m%d%H%M%S), 10800, 3600, 604800, 3600
